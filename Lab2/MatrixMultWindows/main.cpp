...
DWORD WINAPI multiplyMatricesBlocksInPlace(void *);
...
Matrix multiplyMatrices(const Matrix &a, const Matrix &b, int blockSize) {
    ...
    std::vector<HANDLE> threads(blocks * blocks * blocks);
    std::vector<std::vector<HANDLE> > mutexes(blocks, std::vector<HANDLE>(blocks));
    for (int i = 0; i < blocks; ++i) {
        mutexes[i].resize(blocks);
        for (int j = 0; j < blocks; ++j) {
            mutexes[i][j] = CreateMutex(nullptr, false, nullptr);
        }
    }
    Matrix result(n, Array(n));
    for (int i = 0; i < blocks; ++i) {
        for (int j = 0; j < blocks; ++j) {
            for (int k = 0; k < blocks; ++k) {
                ...
                threads[index] = CreateThread(nullptr, 0, multiplyMatricesBlocksInPlace, ta, 0, nullptr);
            }
        }
    }
    for (auto &t: threads) {
...
DWORD WINAPI multiplyMatricesBlocksInPlace(void *args) {
